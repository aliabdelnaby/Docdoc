import '../../../../core/utils/assets.dart';
import '../models/static/reecommendation_doctor_item_model.dart';

List<ReecommendationDoctorItemModel> reecommendationDoctorList = [
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor10,
    rateAndReviews: '4.8 (1,220 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor2,
    rateAndReviews: '4.8 (1,110 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor3,
    rateAndReviews: '4.7 (1,012 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor6,
    rateAndReviews: '4.7 (950 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor5,
    rateAndReviews: '4.9 (1,480 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor4,
    rateAndReviews: '4.5 (850 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor7,
    rateAndReviews: '4.9 (1,780 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor8,
    rateAndReviews: '4.4 (780 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor9,
    rateAndReviews: '4.8 (1,147 reviews)',
  ),
  ReecommendationDoctorItemModel(
    image: Assets.imagesDoctor1,
    rateAndReviews: '4.3 (690 reviews)',
  ),
];
